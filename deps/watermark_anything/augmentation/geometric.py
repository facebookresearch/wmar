...
from deps.watermark_anything.augmentation.valuemetric import JPEG, Brightness
...
class UpperLeftCropWithResizeBack(nn.Module):
    ...
    def forward(self, image, mask, crop_size=None):
        output_size = (image.shape[-2], image.shape[-1])
        image, mask = self.crop(image, mask, crop_size)
