...
from deps.saberi_wmr.DiffPure.guided_diffusion.script_util import (
...
def get_imagenet_class_name(filename):
    class_id = filename.split('_')[0]
    return IMAGENET2012_CLASSES[class_id].split(',')[0]
...
class ImageCaptioner():
    def __init__(self, image_size=256, device="cuda"):
        ...
        self.model = blip_decoder(pretrained=model_url, image_size=image_size, vit='base', med_config='BLIP/configs/med_config.json')
        self.model.eval()
    ...
    def __call__(self, img):
        ...
        return self.model.generate(transform(img).unsqueeze(0).to(self.device), sample=True, top_p=0.9, max_length=20, min_length=10)[0]
...
class ModifiedStableDiffusionPipeline(StableDiffusionPipeline):
    ...
    def __call__(
        ...
        # 4. Prepare timesteps
        if num_pass_steps == -1:
            num_pass_steps = num_inference_steps
        self.scheduler.set_timesteps(num_inference_steps, device=device)
        timesteps = self.scheduler.timesteps
        timesteps = timesteps[-num_pass_steps:]
        ...
        # 7. Denoising loop
        num_warmup_steps = 0 # len(timesteps) - num_inference_steps * self.scheduler.order
...
class InversableStableDiffusionPipeline(ModifiedStableDiffusionPipeline):
    ...
    def backward_diffusion(
        ...
        **kwargs,
        ...
        timesteps_tensor = timesteps_tensor[-num_pass_steps:]
...
class GuidedDiffusion(torch.nn.Module):
    def __init__(self, config, t, device=None, model_dir='pretrained/guided_diffusion'):
        ...
        sd_path = os.path.join(model_dir, "256x256_diffusion_uncond.pt")
        model.load_state_dict(torch.load(sd_path, map_location='cpu'))
        # model.load_state_dict(torch.load(f'{model_dir}/512x512_diffusion_uncond_finetune_008100.pt', map_location='cpu'))
