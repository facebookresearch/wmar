...
from deps.chameleon.inference.utils import DynamicGenerator, advance, random_unused_port
...
from wmar.watermarking.gentime_watermark import (
    GentimeWatermark,
    create_watermarker_from_string,
)
...
class Options:
    ...
    txt: Text | bool = True
    img: Image | bool = True
    extra_eos_tokens: list[int | str] = field(default_factory=list)
...
class TokenManager:
    ...
    def pil_from_bpe_tokens(self, bpe_tokens: torch.Tensor, return_imagetensor: bool = False) -> Union[PIL.Image, tuple[PIL.Image, torch.Tensor]]:
        ...
        return self.image_tokenizer.pil_from_img_toks(image_tensor, return_chw=return_imagetensor)
    ...
    def decode_image(self, ids: torch.LongTensor, return_imagetensor: bool = False) -> list[PIL.Image] | list[tuple[PIL.Image, torch.Tensor]]:
        ret = [self.pil_from_bpe_tokens(sample, return_imagetensor) for sample in ids]
        if return_imagetensor:
...
class TextDecoder(Decoder):
    def __init__(
        ...
        watermark_logit_processor: Callable[[torch.Tensor, torch.Tensor], torch.Tensor] | None = None,
    ):
        self.vocab = vocab
        ...
        self.gen = ChameleonGenerator(
            ...
            logits_processors=self._logits_processors(watermark_logit_processor),
    ...
    def _logits_processors(self, watermark_logit_processor: Callable[[torch.Tensor, torch.Tensor], torch.Tensor] | None = None) -> list[LogitsProcessor]:
        logits_processors = [] 
        if watermark_logit_processor is not None:
            logits_processors.append(watermark_logit_processor)
        logits_processors.append(
            AllowOnlyTokensLogitsProcessor(self._allowed_tokens()),
        )
        if isinstance(self.options.img, Options.Image): # not here 
            logits_processors += [
                DisallowTokensAtOrAfterIndexLogitsProcessor(
                    [self.vocab.begin_image],
...
class Generator(Decoder):
    def __init__(
        ...
        input_ids: list[list[int]],
        watermark_logit_processor: Callable[[torch.Tensor, torch.Tensor], torch.Tensor] | None = None,
        watermark_logit_processor_text: Callable[[torch.Tensor, torch.Tensor], torch.Tensor] | None = None,
    ):
        ...
        self.options = options
        self.watermark_logit_processor_text = watermark_logit_processor_text
        self.watermark_logit_processor_image = watermark_logit_processor
        if not self.options.txt:
            self.dyngen = DynamicGenerator(
                ImageDecoder(model, vocab, options, input_ids, self.watermark_logit_processor_image)
            )
        else:
            self.dyngen = DynamicGenerator(
                TextDecoder(model, vocab, options, input_ids, self.watermark_logit_processor_text)
            )
    ...
    def __next__(self) -> ChameleonGenerator.Token:
        ...
        if piece.next_decoder is not None:
            ...
            self.dyngen.gen = piece.next_decoder(
                ...
                self.options,
                self.input_ids,
                self.watermark_logit_processor_text if piece.next_decoder == TextDecoder else self.watermark_logit_processor_image
            )
        return piece.token
...
def _worker_impl(
    ...
    watermarker_str: str | None = None,
    watermarker_str_text: str | None = None,
    vq_dict: dict | None = None,
):
    ...
    print(f"Initializing Chameleon worker {rank} with init_method {init_method}")
    torch.set_default_device(f"cuda:{rank}")
    torch.cuda.set_device(rank)
    if isinstance(model, str):
        model = loader.load_model(model, rank=rank).to(f"cuda:{rank}")
        model.device = f"cuda:{rank}"
    vsz = vq_dict["vocab_size"]
    watermarker = create_watermarker_from_string(vq_dict, vsz, watermarker_str, device=f"cuda:{rank}") if watermarker_str else None
    if watermarker_str_text is not None:
        watermarker_text = create_watermarker_from_string(vq_dict, vsz, watermarker_str_text, device=f"cuda:{rank}")
    ...
    while True:
        ...
        options, input_ids, key, shutdown, apply_watermark = req
        if shutdown:
            break

        watermark_logit_processor = watermarker.spawn_logit_processor() if apply_watermark else None
        if watermarker_str_text is not None:
            watermark_logit_processor_text = watermarker_text.spawn_logit_processor() if apply_watermark else None
        else:
            watermark_logit_processor_text = None
        for token in Generator(
            model=model,
            vocab=vocab,
            options=options,
            input_ids=input_ids,
            watermark_logit_processor=watermark_logit_processor,
            watermark_logit_processor_text=watermark_logit_processor_text,
        ):
            if is_coord:
...
class ChameleonInferenceModel:
    def __init__(
        ...
        *,
        ...
        self.world_size = 1
        if isinstance(model, str):
            self.world_size = loader.detect_shard_count(model)
        self.dctx = _DistributedContext.make(distributed_mode, self.world_size)
        self.model = model 
    
    def set_watermarker_and_init_workers(self, watermarker: GentimeWatermark | None = None, watermarker_text: GentimeWatermark | None = None):
        self.watermarker = watermarker
        self.watermarker_text = watermarker_text
        watermarker_str = str(watermarker) if watermarker else None
        watermarker_str_text = str(watermarker_text) if watermarker_text else None

        # Now start workers
        init_method = f"tcp://0.0.0.0:{random_unused_port()}"
        
        actual_vq = self.token_manager.image_tokenizer._vq_model.quantize
        vq_dict = {
            "vocab_size": len(self.token_manager.vocab.all_tokens),
            "alive_ids": actual_vq.alive_ids.detach().cpu(),
            "dead_ids": actual_vq.dead_ids.detach().cpu(),
            "embedding": actual_vq.embedding.weight.detach().cpu()
        }
        
        self.workers = [
            self.dctx.worker_launcher(
                target=_worker_impl,
                args=(init_method, self.model, self.world_size, i, self.vocab, self.dctx, watermarker_str, watermarker_str_text, vq_dict),
                daemon=True,
            )
    ...
    def stream(
        ...
        *,
        ...
        self.dctx.req_q.put([options, batch_input_ids, req_key, False, apply_watermark])
        ...
        try:
            ...
            while key_token := self.dctx.res_q.get():
                count += 1
                key, token = key_token
                #print(count, f"memory: {torch.cuda.memory_allocated() / 1024**2:.2f}MB")
                if key != req_key:
    ...
    def decode_image(self, ids: torch.LongTensor, return_imagetensor: bool = False) -> list[PIL.Image] | list[tuple[PIL.Image, torch.Tensor]]:
        return self.token_manager.decode_image(ids, return_imagetensor)
