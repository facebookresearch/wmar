...
from deps.chameleon.inference.alignment import AlignPromptLeft, PromptAlignment
from deps.chameleon.inference.model_adapter import ModelAdapter
from deps.chameleon.inference.stopping_criteria import StoppingCriteria, StoppingCriteriaList
from deps.chameleon.inference.token_selector import MultinomialTokenSelector, TokenSelector
...
class ChameleonGenerator:
    ...
    def __init__(
        ...
        self._inputs = self.alignment.prepare_inputs(
            input_ids
        ).to(self.model.device)  # inputs.shape = [batch, seq-len]
